...
<?= $this->section('title') ?>
Dashboard CC - Llamadas Por Año - - <?= implode(",", $params['queue']) ?> <small>(<?= $params['inicio'] ?> a <?= $params['fin'] ?>)</small> <span><small>(Last Update: <?= $lastUpdate ?>)</small></span>
<?= $this->endSection() ?>
        ...
        <div class="card card-container" data-copy-target="total-calls-card">
            ...
            <div id="total-calls-card" class="card-body">
                ...
                <p class="text-center" style="font-size: 1.5rem; font-weight: bold;">
                    <?= number_format($totals['totalLlamadas'], 0, '.', ',') ?>
                </p>
                <div class="d-flex">
                    <div class="me-4">
                        <small class="text-muted">Ans: <?= number_format($totals['Answered'], 0, '.', ',') ?></small><br>
                        <small class="text-muted">Early: <?= number_format($totals['EarlyAbandon'], 0, '.', ',') ?></small><br>
                    </div>
                    <div class="">
                        <small class="text-muted">Abn: <?= number_format($totals['Abandon'], 0, '.', ',') ?></small><br>
                        <small class="text-muted">Tfr: <?= number_format($totals['Transferida'], 0, '.', ',') ?></small><br>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card card-container" data-copy-target="abandon-card">
            ...
            <div id="abandon-card" class="card-body">
                ...
                <p class="card-text"><?= 
                    ...
                    $totals['Transferida'] + $totals['Answered'] + $totals['Abandon'] == 0 ? 0 :
                    round(($totals['Abandon'] / ($totals['Transferida'] + $totals['Answered'] + $totals['Abandon'])) * 100, 2) 
                ?>%</p>
            </div>
        </div>
    </div>
